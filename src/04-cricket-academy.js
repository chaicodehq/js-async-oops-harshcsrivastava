...
export class Batsman extends Player {
    ...
    getStrikeRate() {
        ...
        const sum = this.innings.reduce(
            (sum, inn) => (sum += inn.strikeRate),
            0,
        );
        const avgStrikeRate = sum / this.innings.length;
...
export class Bowler extends Player {
    ...
    getEconomy() {
        ...
        const sum = this.spells.reduce((sum, sp) => (sum += sp.economy), 0);
        const avgEconomy = sum / this.spells.length;
        return avgEconomy;
    }
